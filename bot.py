...
SPAWN_EVERY = 20  # CHANGED: Now spawns every 20 messages
BID_DURATION = 60
BID_EXTEND = 10
TRADE_TIMEOUT = 120
USER_MSG_COOLDOWN = 5
MIN_MSG_LENGTH = 1  # CHANGED: Now counts any message (1 letter minimum)
ITEMS_PER_PAGE = 5
...
last_message_text = {}
user_locks = {}
spawn_counter = 0  # Global spawn counter that increments for each spawn
...
async def wtop_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    period = data.split("_")[1]  # daily, weekly, or alltime
    
    # Calculate wins based on period
    user_wins = []
    now = datetime.now()
...
async def expire_trade(tid, context):
    ...
    # Check if trade still exists before popping
...
async def spawn_character(chat_id, context):
    ...
    active_bid[chat_id] = {
        ...
        "spawn_number": spawn_counter  # Store spawn number
    }
    rl = rarity_label(rarity)
    caption = (
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Spawn counter for group chats - now counts ALL messages (numbers, words, any length)
    if update.effective_chat.type in ("group", "supergroup"):
        now = time.time()
        key = (chat_id, user_id)
        
        # Count ANY message - removed letter requirement
    ...
    # Wordle - now validates against word list
    text_lower = text.lower().strip()
    words = load_words_once()
    word_set = set(words)
    
    if len(text_lower) == 5 and text_lower.isalpha() and text_lower in word_set and user_id in active_wordle:
        ensure_user(user_id, update)
        game = active_wordle[user_id]
        answer = game["answer"]
