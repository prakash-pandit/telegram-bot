...
# ═══════════════════════════════════════════════════════════════
# WORDLE IMAGE GENERATION - FIXED (Letters fill the boxes)
# ═══════════════════════════════════════════════════════════════
...
def generate_wordle_image(guesses, total_attempts=6):
    ...
    for row in range(total_attempts):
        for col in range(5):
            ...
            # Draw letter BIG and filling the box
            if letter:
                draw.text((x + 32, y + 28), letter, fill=COLOR_TEXT)
...
async def test_image(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        draw = ImageDraw.Draw(img)
        draw.text((50, 80), "Image Generation Works!", fill=(255, 255, 255))
        draw.text((50, 120), "Wordle images will appear here", fill=(200, 200, 200))
        
        buf = io.BytesIO()
        img.save(buf, format="PNG")
        buf.seek(0)
        
        await update.message.reply_photo(photo=buf, caption="✅ Image generation is working!")
    except Exception as e:
        await update.message.reply_text(f"❌ Image failed: {e}")
