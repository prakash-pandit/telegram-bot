...
# NEW: Table format for Wordle (colors on top, letters below)
def format_wordle_table(guesses):
    lines = []
    for word, emojis in guesses:
        word_upper = word.upper()
        # Color row (top)
        color_row = '  '.join(emojis)
        # Letter row (bottom)
        letter_row = '  '.join(word_upper)
        lines.append(f"`{color_row}`")
        lines.append(f"`{letter_row}`")
    return '\n'.join(lines)
