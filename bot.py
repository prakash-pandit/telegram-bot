...
def load_words_once():
    ...
    try:
        ...
        if words:
            WORD_LIST = words
            logging.info(f"Loaded {len(words)} words from word.txt")
            return WORD_LIST
    except Exception as e:
        logging.warning(f"Could not load word.txt: {e}")
    ...
    logging.info(f"Using fallback word list: {len(WORD_LIST)} words")
    return WORD_LIST

# ═══════════════════════════════════════════════════════════════
# WORDLE IMAGE GENERATION (COMPLETELY REWRITTEN)
# ═══════════════════════════════════════════════════════════════

def get_font_safe(size, bold=False):
    """Safely get font, fallback to default if not found"""
    try:
        if bold:
...
def generate_wordle_image(guesses, total_attempts=6):
    ...
    # Cell dimensions - BIGGER for better readability
    CELL_SIZE = 90
    CELL_GAP = 10
    PADDING = 25
    HEADER_HEIGHT = 65
    ...
    # Load fonts
    font_title = get_font_safe(32, bold=True)
    font_letter = get_font_safe(58, bold=True)

    # Draw title
    title_bbox = draw.textbbox((0, 0), "WORDLE", font=font_title)
    title_width = title_bbox[2] - title_bbox[0]
    draw.text(((width - title_width) // 2, 18), "WORDLE", fill=COLOR_TEXT, font=font_title)
    ...
    for row in range(total_attempts):
        for col in range(5):
            ...
            y = PADDING + HEADER_HEIGHT + row * (CELL_SIZE + CELL_GAP)
            
            # Get color and letter for this cell
            if row < len(guesses):
                word, emojis = guesses[row]
                color = color_map.get(emojis[col], COLOR_GRAY)
            ...
            draw.rectangle([x, y, x + CELL_SIZE, y + CELL_SIZE], fill=color, outline=COLOR_BORDER, width=2)
            ...
            if letter:
                ...
                draw.text(
                    (x + (CELL_SIZE - letter_width) // 2, y + (CELL_SIZE - letter_height) // 2),
                    letter,
                    fill=COLOR_TEXT,
                    font=font_letter
                )
...
def get_wordle_emoji_list(guess, answer):
    """Generate emoji list for a wordle guess"""
    guess = guess.lower()
    answer = answer.lower()
    result = ['⬜'] * 5
...
async def test_image(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Test if image generation works (owner only)"""
    ...
    try:
        ...
        img = Image.new("RGB", (400, 200), (50, 50, 80))
        draw = ImageDraw.Draw(img)
        ...
        draw.text((50, 80), "✅ Image Generation Works!", fill=(255, 255, 255), font=font)
        
        buf = io.BytesIO()
        img.save(buf, format="PNG")
        buf.seek(0)
        
        await update.message.reply_photo(photo=buf, caption="✅ Image generation is working properly!")
    except Exception as e:
...
def save_data():
    ...
    active_wordle_clean = {}
    for uid, game in active_wordle.items():
        game_copy = game.copy()
        if "excluded_letters" in game_copy and isinstance(game_copy["excluded_letters"], set):
            game_copy["excluded_letters"] = list(game_copy["excluded_letters"])
        active_wordle_clean[uid] = game_copy
    ...
    data = {
        ...
        "active_wordle": active_wordle_clean,
...
async def debug_spawn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Debug command to check spawn status (owner only)"""
    if str(update.effective_user.id) != str(OWNER_ID):
        await update.message.reply_text("❌ Owner only.")
        return
...
async def force_spawn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Force spawn a character (owner only)"""
    if str(update.effective_user.id) != str(OWNER_ID):
        await update.message.reply_text("❌ Owner only.")
        return
...
async def reset_spawn_counter(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Reset spawn counter for current group (owner only)"""
    if str(update.effective_user.id) != str(OWNER_ID):
        await update.message.reply_text("❌ Owner only.")
        return
...
async def wordle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "continue_wordle":
        await query.message.delete()
        await query.message.reply_text("✅ Continue playing! Type your next 5-letter guess.")
        
    elif query.data == "cancel_wordle":
        if user_id in active_wordle:
            answer = active_wordle.pop(user_id)["answer"]
            save_data()
            await query.message.delete()
            await query.message.reply_text(
                f"❌ Previous game cancelled!\n\nThe word was: <b>{answer.upper()}</b>\n\n"
                f"Use /wordle to start a new game!",
                parse_mode="HTML"
            )
        else:
            await query.message.edit_text("No active game found!")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "/testimage - Test image generation (owner)\n"
        "/help - All commands",
        parse_mode="HTML"
    )
...
async def wordle_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in active_wordle:
        ...
        keyboard = [[
            InlineKeyboardButton("❌ Cancel Old Game", callback_data="cancel_wordle"),
            InlineKeyboardButton("▶️ Continue Playing", callback_data="continue_wordle")
        ]]
        
        await update.message.reply_photo(
            photo=img_buf,
            caption=(
                f"⚠️ <b>You already have an active Wordle game!</b>\n\n"
                f"📊 Attempts used: {len(game['guesses'])}/6\n"
                f"❌ Remaining: {6 - len(game['guesses'])} guesses\n\n"
    ...
    active_wordle[user_id] = {"answer": secret, "guesses": [], "excluded_letters": []}
    save_data()

    await update.message.reply_text(
        "🎮 <b>WORDLE STARTED!</b>\n\n"
        "⚠️ <b>ANY 5-letter word works!</b>\n\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(text_lower) == 5 and text_lower.isalpha() and user_id in active_wordle:
        ...
        excluded_letters = set(game.get("excluded_letters", []))
        ...
        for i, emoji in enumerate(emoji_list):
            if emoji == '⬜' and text_lower[i] not in game["excluded_letters"]:
                game["excluded_letters"].append(text_lower[i])
...
async def main():
    ...
    logging.info(f"✅ Word list: {len(WORD_LIST)} secret words")
    ...
    application.add_handler(CallbackQueryHandler(wordle_callback, pattern="^(cancel_wordle|continue_wordle)$"))
