...
async def inline_query(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, char in enumerate(collection[:50]):
...
def index():
    return jsonify({"status": "Bot is running", "message": "Use /health for status"}), 200
...
async def main():
    ...
    application.add_handler(CallbackQueryHandler(ctop_callback, pattern="^ctop_"))
    ...
    await application.updater.start_polling()  # This won't actually poll because we're using webhook
    # Keep alive
    while True:
