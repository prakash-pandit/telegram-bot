...
def load_data():
    ...
    global last_checkin, checkin_streak, uploaders
    try:
...
def load_words():
    try:
        ...
        pass
    return ["apple", "brain", "crane", "dance", "eagle", "flame",
...
async def mycards(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not collection:
        ...
        return
    text = f"📚 <b>Your Collection ({len(collection)} cards)</b>\n\n"
    for i, c in enumerate(collection[:15], 1):
        fav = "⭐ " if c.get("favourite") else ""
        text += f"{i}. {fav}{html.escape(c.get('name','?'))} — {rarity_label(c.get('rarity',1))} | #{c.get('card_id','?')}\n"
...
async def summon(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if users[user_id].get("coins", 0) < cost:
        await update.message.reply_text(f"❌ Not enough coins! You need {cost:,} coins.")
        return
...
async def wtop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    sorted_users = sorted(
        users.items(),
        key=lambda x: x[1].get("wordle_wins", {}).get("alltime", 0),
        reverse=True
    )
    text = "🏆 <b>Wordle Leaderboard (All Time)</b>\n\n"
    for i, (uid, d) in enumerate(sorted_users[:10], 1):
        wins = d.get("wordle_wins", {}).get("alltime", 0)
        if wins == 0:
            continue
...
async def shop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    buy_buttons.append([InlineKeyboardButton("❌ Close", callback_data="shop_close")])

    await update.message.reply_text(text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(buy_buttons))
...
async def shop_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("shop_buy_"):
        ...
        coins = users[user_id].get("coins", 0)
        if coins < price:
            await query.answer(f"❌ Not enough coins! Need {price:,}, have {coins:,}.", show_alert=True)
            return
        users[user_id]["coins"] -= price
        users[user_id]["characters"].append(char)
        save_data()
        await query.answer(f"✅ Bought {char.get('name','?')} for {price:,} coins!", show_alert=True)
...
async def resolve_bid(chat_id, context):
    ...
    if not top_bidder:
        await context.bot.send_message(chat_id=chat_id, text=f"⌛ No bids for <b>{html.escape(character.get('name','?'))}</b>! Character lost.", parse_mode="HTML")
        return
    uid = top_bidder
    ensure_user(uid)
    if users[uid]["coins"] < top_bid:
        await context.bot.send_message(chat_id=chat_id, text="⚠️ Winner didn't have enough coins! Card lost.", parse_mode="HTML")
        return
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(text_lower) == 5 and text_lower.isalpha() and user_id in active_wordle:
        ...
        game = active_wordle[user_id]
        answer = game["answer"]
        emoji = get_wordle_emoji(text_lower, answer)
        game["guesses"].append((text_lower, emoji))
        save_data()
...
async def run_bot():
    ...
    bot_loop = asyncio.get_running_loop()  # ✅ CRITICAL FIX
    print("✅ Bot running!")
...
if __name__ == "__main__":
    ...
    # Give Flask time to bind before bot starts
    time.sleep(2)
    asyncio.run(run_bot())
