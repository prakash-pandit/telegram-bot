...
# ═══════════════════════════════════════════════════════════════
# SAVE / LOAD FUNCTIONS (FIXED for MongoDB)
# ═══════════════════════════════════════════════════════════════
...
def save_data():
    ...
    # Convert active_wordle to MongoDB-friendly format (convert lists, not sets)
    active_wordle_clean = {}
    for uid, game in active_wordle.items():
        game_copy = game.copy()
        # excluded_letters should already be a list, but just in case
...
def load_data():
    ...
    try:
        ...
        if data:
            ...
            # Convert excluded_letters from list back to list (keep as list for MongoDB compatibility)
            for uid, game in active_wordle.items():
                if "excluded_letters" in game and isinstance(game["excluded_letters"], list):
                    # Keep as list, don't convert to set
                    pass
                elif "excluded_letters" in game and isinstance(game["excluded_letters"], set):
                    game["excluded_letters"] = list(game["excluded_letters"])
...
async def wordle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # FIXED: Use list instead of set for excluded_letters
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # WORDLE - With IMAGE generation (FIXED for list instead of set)
    ...
    if len(text_lower) == 5 and text_lower.isalpha() and user_id in active_wordle:
        ...
        # Convert excluded_letters list to set for checking
        ...
        # Update excluded letters - append to list (not set)
